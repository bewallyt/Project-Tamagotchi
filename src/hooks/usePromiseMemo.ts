...
import { dependenciesMatch } from 'utils';
...
const usePromiseMemo = <T, E = unknown>(
  ...
  const checkIfPromiseIsStillValid = useCallback((dependenciesAtTimeOfPromise: unknown[]): boolean => {
    return isMounted.current && dependenciesMatch(dependenciesAtTimeOfPromise, dependencies.current);
  }, []);
